...
class MinException(FieldException):
    pass


class MaxException(FieldException):
    pass
...
def parse_phone(number):
    try:
        ...
        res = phone_pattern.search(number).groups()
        st = "".join(res)
        if st.startswith("1"):
            return "+%s" % st
        else:
...
class Char(Field):
    ...
    def clean(self, val, doc=None):
        try:
            val = self._type(val)
            if self._max and len(val) > self._max:
                raise MaxException("must be less than %s" % self._max)
            if self._min and len(val) < self._min:
            ...
            raise FieldException(
                "%s is not a valid %s" % (val, self._exception_display))
...
class Boolean(Field):
    ...
    def clean(self, val, doc=None):
        try:
            ...
            raise FieldException("%s invalid boolean" % val)
        ...
        try:
            if isinstance(val, list):
                if len(val) == 2:
                    ...
                    raise FieldException("to many values: %s" % val)
            else:
                raise FieldException("must be type array")
        except Exception:
            raise FieldException("%s must be array" % val)
...
class DocumentId(Field):
    ...
    def __call__(self):
        if self._value is not None and self._type:
            return self._type(id=self._value)
        else:
            raise FieldException(
                "Cannot instantiate %s with id %s" % (self._type, self._value))
...
class AutoIncrement(Integer):
    ...
    def _save(self, namespace):
        if self._value is None:
            ...
            res = self._conn["auto_increment"][col].find_and_modify(
                {"field": self._name}, {"$inc": {"val": 1}},
                upsert=True, new=True, fields={"val": True}
                )
            self._value = res['val']
    ...
    def clean(self, val, doc=None):
        if isinstance(val, Document):
            if val._id:
                ...
                raise FieldException(
                    "Document does not have an id. Be sure to save first.")
        elif isinstance(val, dict):
            return val
        else:
            raise FieldException(
                "%s is not a valid document type" % val.__class__.__name__)
...
class Choice(Char):
    ...
    def clean(self, val, doc=None):
        ...
        vals = [opt['value'] if isinstance(
            opt, dict) else opt for opt in self._choices]
        if val not in vals:
            raise FieldException("%s is not a valid option")
        return val
...
class ModelChoice(DocumentId):
    ...
    def get_choices(self, render=None):
        if render:
            ...
            raise FieldException("no render method available")
...
class CollectionChoice(Choice):
    ...
    def get_choices(self, render=None):
        if render:
            cur = self._conn[self._db][self._collection].find(
                self._query, fields=self._fields)
            cur = cur.sort(self._sort) if self._sort else cur
            return [render(i) for i in cur]
        else:
            raise FieldException("no render method available")
...
class Regex(Char):
    ...
    def clean(self, val, doc=None):
        ...
        if not self._reg.search(val):
            raise FieldException(
                "%s: pattern not found" % (
                    val if not self._disp_error else self._disp_error))
        return val
...
class Phone(Char):
    ...
    def clean(self, val, doc=None):
        ...
        try:
            ...
            raise FieldException("%s is not a valid format" % val)
...
class File(DocumentId):
    ...
    def clean(self, val, doc=None):
        if not self._database:
            raise FieldException("database is required")
