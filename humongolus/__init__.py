...
class FieldValidator(object):
    ...
    def validate(self, val, doc=None):
        ...
        :Parameters:
            ...
            - `doc`: a dictionary available if using the Form system.
                     The dictionary will contain all keys passed to
                     the Form object on instantiation
        """
        return val
...
class Attributes(object):
    ...
    def name(self):
        return "%s_%s" % (
            self.prepend, self._name) if self.prepend else self._name
...
class Widget(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.errors = []
        if self._prepend and 'prepend' not in kwargs:
            kwargs['prepend'] = self._prepend
        else:
    ...
    def clean(self, val, doc=None):
        ...
        Should always return the "cleaned" value or raise a
        FieldException on error

        :Parameters:
            - `val`: incoming value from a form or other assignment
            - `doc`: a dictionary available if using the Form system.
                     The dictionary will contain all keys passed to the
                     Form object on instantiation
        """
        return val
...
class Field(object):
    ...
    :Parameters:
        - `name`: when used in a Model, this will be the
           attribute name, optional
        - `default`: default value, optional
        - `required`: whether or not the field is required,
           optional default to False
        - `display`: value for display purposes, this is used for labels
           when using the :class: `~widget.Form` widget, optional
        - `dbkey`: if the field name in the mongodb document is different
           then the Models attribute name, use this to indicate the
           mongo attribute name
        - `widget`: :class: `~Widget` to use for rendering,
           also assignable when creating a form, optional
        - `validate`:an instance :class:
          `~FieldValidator` for custom validation
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._dirty = self.clean(
            self._default, doc=None) if self._default not in EMPTY else None
        self._value = self.clean(
            self._default, doc=None) if self._default not in EMPTY else None
        self._error = None
    ...
    def _clean(self, val, dirty=None, doc=None):
        ...
        val = self._validate(self).validate(
            val, doc=doc) if self._validate else val
    ...
    def clean(self, val, doc=None):
        ...
        :Parameters:
            - `val`: incoming value from a form or other assignment
            - `doc`: a dictionary available if using the :class:
                    `~Form` system. The dictionary will contain all
                    keys passed to the :class: `~Form` object on instantiation
        """
        return val

    def _isrequired(self, val):
        if val in EMPTY and self._required:
            raise FieldException("Required Field")
...
class Lazy(object):
    ...
    def __init__(self, *args, **kwargs):
        """
        :Parameters:
            - `type`: the type of :class: `~Document` returned.
                      must be an instance of :class: `~Document`
            - `key`: the "foreign key" to look up the types by.
                     must be an attribute of type.
        """
        self.logger = _settings.LOGGER
        self.__args__ = args
        self.__kwargs__ = kwargs
    ...
    def __call__(self, **kwargs):
        """when calling the lazy object it will return a mongodb cursor
        that yields models of the type.
        It will use the the _id of the base document
        and look in the key of the type class
        """
        q = kwargs.pop('query', {})
...
class List(list):
    """Used to describe an array in a :class: `~Document` or
    :class: `~EmbeddedDocument`. Extends list."""
    logger = None
    _type = None
    _length = None
    _dbkey = None
    ...
    def __init__(self, *args, **kwargs):
        ...
        :Parameters:
            - `type`: the type of :class: `~Document` returned.
            must be an instance of :class: `~Document`, can be an array
            - `length`: maximum length of the array
            - `dbkey`: if the field name in the mongodb
            document is different then the Models attribute name,
            use this to indicate the mongo attribute name
            - `widget`: :class: `~Widget` to use for rendering,
            also assignable when creating a form, optional
        """
        self.logger = _settings.LOGGER
    ...
    def append(self, obj):
        ...
        if obj.__class__ in types:
            ...
            raise Exception("%s not of type %s" % (
                obj.__class__.__name__, self._type.__name__
                ))
    ...
    def _save(self, namespace):
        ...
        if len(self) == 0:
            return {namespace: []}
        if not self._inited:
            return {namespace: self._json()}
        for id, obj in enumerate(self):
            ns = ".".join([namespace, str(id)])
            try:
                if obj._inited:
                    ret.update(obj._save(namespace=ns))
                else:
                    ret.update({ns: obj._json()})
            except Exception:
                ret[ns] = obj
        return ret
    ...
    def delete(self, query, key):
        ob = self[key]
        self._base._coll.update(
            {'_id': self._base._id}, {'$pull': {query: ob}}
        )
        del self[key]
...
class Document(base):
    """Base class for all first level documents.
    This will contain the _id and has the "save" method.

    All models should extend this class.

    When extending always set _db and _collection.
    These tell humongolus where to save and find it's documents.

    _indexes is used to set the mongo indexes, it should be an array of :class:
    `~Index` objects
    ...
    def _get_doc(self, id):
        return _settings.DB_CONNECTION[self._db][self._collection].find_one(
            {'_id': ObjectId(id)}
            )
    ...
    @property
    def modified(self):
        """return last modify date of document.
        updated after every successful save: datetime
        """
    ...
    def _connection(cls):
        ...
        _coll = mongo.Collection(
            cls, database=_conn[cls._db], name=cls._collection
            )
        return _coll

    @classmethod
    def find(cls, *args, **kwargs):
        """returns pymongo cursor that yields instantiated Document objects
        of the Class type.
        :Parameters:
            - `*args`: passed directly to Connection.find()
            - `**kwargs`: passed directly to Connection.find()

        extra kwargs parameter is as_dict this will return the raw dictionary
        from mongo, this also allows you to use the "fields" parameter
        """
    ...
    def _map(self, vals, *args, **kwargs):
        ...
        self._version = vals.get('_version', self._version)
        try:
    ...
    def save(self):
        """use this method to write a new object to the database
        or to save en existing document after updating.

        if the document already exists it will send an atomic update
        of only the changed attributes to mongo.

        always uses safe=True

        will raise a DocumentException if there are errors from validation,
        will also throw a pymongo Exception if insert or update fails.
        ...
        if not self._id:
            ...
            self.__modified__ = datetime.datetime.utcnow()
            obj['__modified__'] = self.__modified__
            up = {'$set': obj, '$inc': {'_version': 1}}
            ret = self._coll.find_and_modify(
                query={'_id': self._id, "_version":  version},
                update=up, upsert=False, full_response=True
                )
            if ret['value'] is None:
                raise DocumentException("Version mismatch")
        return self._id
...
class Index(object):
    ...
    def create(self, conn):
        if not isinstance(self._key, list):
            self._key = [self._key]
        ob = dict(
        ...
        if self._ttl:
            ob['expireAfterSeconds'] = self._ttl
